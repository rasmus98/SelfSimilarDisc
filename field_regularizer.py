...
class field_regularizer:
    def __init__(self, parameter_rep, settings):
        self.rho, self.v, self.B, self.theta_grid = self.un_flatten(parameter_rep, settings.theta_grid)
        ...
        self.rho_multiplier = settings.Sigma / column_density
        self.rho *= self.rho_multiplier
        
        #Normalize magnetic field strength
        B_target = np.sqrt(8*np.pi*centre(self.rho.v)**2*centre(settings.cs.v)**2/settings.beta)
    ...
    def infer_Br(B_theta, theta_grid):
        expanded_theta = pad(theta_grid, 1, False)
    ...
    def un_flatten(parameter_rep, theta_grid):
        ...
        v = VectorField(*[pad(v, 2) for v in [vr_data, vtheta_data, vphi_data]],
                  -1/2, theta_grid)
        B = VectorField(infer_Br(Btheta_data, theta_grid[2:-2]), *[pad(B, 2, False) for B in [Btheta_data, Bphi_data]], -5/4, theta_grid)
